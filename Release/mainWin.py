...
def get_type(inp):
    type_ch = inp[0]
    if type_ch in ['A', 'P']: return type_ch, inp[2:]
    else: return 'F', inp
...
def sendP(type, nums):
    result = subprocess.run(['COLOCOUS.exe', type, *nums], stdout = subprocess.PIPE)
...
class CalcWin(QMainWindow):
    ...
    def butClicked(self):
        try:
            ...
            if type == 'P':
                send_nums, send_ops = parse_input_pol(inpText)
                out_text = sendP(type, send_nums)
            else:
